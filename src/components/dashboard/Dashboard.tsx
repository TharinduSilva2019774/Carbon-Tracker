...
import { formatCO2Amount, calculateEquivalents } from '@/lib/calculations/carbonFootprint';
    ...
    <div className="bg-white rounded-xl shadow-lg p-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          {change !== undefined && (
            <p className={`text-sm mt-1 ${change > 0 ? 'text-red-600' : change < 0 ? 'text-green-600' : 'text-gray-600'}`}>
              {change > 0 ? '↗' : change < 0 ? '↘' : '→'} {Math.abs(change).toFixed(1)}% from last week
            </p>
          )}
        </div>
        <div className={`text-3xl p-3 rounded-full ${color.replace('text-', 'bg-').replace('-600', '-100')}`}>
          {icon}
        </div>
      </div>
    </div>
...
export default function Dashboard({ dashboardData: propDashboardData, activityHistory = [], onNavigate }: DashboardProps) {
  const { user } = useAuth();
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <h2 className="text-2xl font-bold text-gray-900 mb-2">Welcome to Carbon Tracker!</h2>
          <p className="text-gray-600 mb-6">Start tracking your digital activities to see your carbon footprint.</p>
    ...
    <div className="min-h-screen bg-gradient-to-br from-green-50 to-emerald-100 p-6">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        {dashboardData.equivalents.length > 0 && (
          <div className="bg-white rounded-xl shadow-lg p-8">
            ...
            <div className="grid md:grid-cols-2 gap-4">
              {dashboardData.equivalents.slice(0, 4).map((equivalent, index) => (
                <EquivalentCard
                  key={index}
        ...
        {activityHistory.length > 0 && (
          <div className="bg-white rounded-xl shadow-lg p-8">
            <h3 className="text-2xl font-bold text-gray-900 mb-6 text-center">Recent Activities</h3>
            <div className="space-y-4">
              {activityHistory.slice(-3).reverse().map((entry) => (
                <div key={entry.id} className="bg-gray-50 rounded-lg p-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {Object.entries(entry.activities).map(([activity, value]) => 
                      (value as number) > 0 ? (
                        <span key={activity} className="text-xs bg-blue-100 text-blue-700 px-2 py-1 rounded-full">
